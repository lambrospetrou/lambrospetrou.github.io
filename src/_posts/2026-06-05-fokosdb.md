...
- [Topology encoding and caching](#topology-encoding-and-caching)
  ...
  - [Partial caching with response piggybacking](#partial-caching-with-response-piggybacking)
...
- Each item is identified by a **hash key** and a **sort key**.
  - When items have the same hash key they should have different sort keys, and items having the same hash key are sorted lexicographically based on their sort key allowing range queries within the hash key.
...
- Core operations: `putItem`, `getItem`, `deleteItem`, `queryItems`.
...
I love constraints because they force you to compromise something to benefit on something else.

Let's have some fun. 😉

## System architecture overview

The following diagram contains everything that makes FokosDB work, except the distributed transaction coordinators that we will mention later.
...
flowchart TD
    ...
    H -->|BY_SortKey| H1[(PartitionDO\nrange.HK_ABC.SK_MIN.SK_cde)]
    H -->|BY_SortKey| H2[(PartitionDO\nrange.HK_ABC.SK_cde.SK_ghf)]

    TK(TopologyKeeperDO) -->|topology encoded| C
    G -->|partition split| TK
    D -->|partition split| TK

    classDef hashPartition stroke:#000099,stroke-width:3px
    class D,DD,E,F,FF,FFN hashPartition
    classDef rangePartition stroke:#990000,stroke-width:3px
    class G,H,I,,H1,H2 rangePartition
```

</div>

If the partitions look like they form B+tree, it's because they are a B+tree, albeit unbalanced.

As we will explore below, there are internal nodes acting as routers only forwarding requests, and then leaf nodes that actually have the data and process the queries. There are some nuances around it, but this premise holds in general.
...
Each partition acts as a "hash" partition (blue in the diagram above) or a "range" partition (red in the diagram above). Everything in the schema and behavior is the same, except that hash partitions route requests to their children using the hash key, and the range partitions using the sort key.
...
We still use many Durable Object partitions to scale out the throughput and storage of the database, but we pack many items in each partition to exploit running live DOs allowing low latencies.
...
There are two state machines at play during a partition split:

1. The parent partition being split tracks the split in `spit_status`:
   1. Once we need a split, it sets `SPLIT_QUEUED` and starts a background job.
   2. The background job attempts to initialize the child partition Durable Objects with the right context and only proceed if all of them are initialized. This is idempotent and can be retried if something fails.
   3. Once all child partitions are initialized, status moves to `SPLIT_STARTED`. From this point forward, all item operation requests are forwarded to the child partitions.
   4. Once all the child partitions migrated their data and acknowledged their completion, status moves to `SPLIT_COMPLETED` and can cleanup data no longer needed.
2. The child partition created tracks the split in `migration_status`:
   1. When the parent initializes the child DO, status is set to `MIGRATION_INITIALIZED` and waits for the signal from the parent to start migrating data.
   2. Once notified to migrate data, it sets `MIGRATION_MIGRATING` and starts a background job that will start pulling data. During this status all requests are handled in the child partition: writes are rejected, whereas reads are re-routed back to the parent again and read from there, since we don't yet have the full data locally.
   3. Once all data is migrated, migration status is set to `MIGRATION_COMPLETE`, it signals the parent as complete, and going forward processes all requests locally.
...
sequenceDiagram
      ...
      P->>P: checkSplits(hk, sk)<br/>maybeQueueSplit() + scheduleBackgroundWork(10ms)<br/>split_status → SPLIT_QUEUED<br/>
      P-->>C: PutItemResult
      deactivate P

      Note over P,BG: ── PHASE 1: Split Init (background job) ──
      ...
      activate Ch
      Note over Ch: Persist parent/split ctx<br/>migration_status → MIGRATION_INITIALIZED
      Ch-->>BG: ok
      deactivate Ch
      Note over BG: All Child DOs initialized, split_status → SPLIT_STARTED
      ...
      activate Ch
      Note over Ch: migration_status → MIGRATION_MIGRATING
      Ch-->>BG: ok
      deactivate Ch
      deactivate BG

      Note over P,Ch: ── PHASE 2: Child Data Migration (background job) ──
      ...
      Note over Ch: ── PHASE 3: Cutover ──<br/>Processes reads and writes locally<br/>migration_status → MIGRATION_COMPLETED
      ...
      alt All children acknowledged
          Note over P: split_status → SPLIT_COMPLETED
      end
      P-->>Ch: ok
      deactivate P
```

[Mermaid LIVE EDITOR](https://mermaid.live/edit#pako:eNqVVn9P40YQ_Soj_3OOmkB-EGitFok4aWodhFzI9aQrFdrYg-2L7XV315AcQrq_-gGq-4T3STq7tkM4gg4QgXg9O2_nzZtn31k-D9ByLIn_FJj5OIxZKFh6mYH5yZlQsR_nLFPgApPgJjFmatftqb491Qsq5tnwfFfMYKyDThImUtgHiWoep8gLBfbWxsZO8MigR3ESbIPYkqLreLd1fDx1IC-UpzC1o2UTJH0CptgmJ_NVfMMUwrRemXC64jcogPZ-EDFd0ScFxc1_CYotEqyjpyWGH6G_vMiTWMkKp_HrQuwfp2y9wHcFFmhu2g34CSQFB0WCA-YvQ8GLLPjAxdLutFNZbpI69EoqpgoJ3_79Dy6mp9786t370fvR0EQ8oBO868C0LHGGskg2zQhwu7gd5TUHYwe-ff1CvzD94-RiBB0HzDnBy-iPvdicED7xRaOKfci1yT8Y10uDMZ1I5w1Qoa8eHd3UQmUJVZHx9Ex6p8aOWRJ_RqJV93d4LsGOA0xzistU4xGWGzkQ047fBU_LtHt7e01Q6xx_exMxGb152ms3eoqs80xRyFgqLTTC2Td9AF-tylbGNAhaZdudOfPGs5O5dz658ibe3Ds59T6Ohhv9Ra2KDL7c0ZRdp9DBJ0lSCVsXHm_YCEhWu5VxMT-ZzR9wt1tsyjLlUDbgWbKGay5umQgkCD3jUkkIeZyF9bop1uheR3yiJmIAdr_da8AtnQqhIiILTeQMmUnVIp3oSK0ZPSslh3ATMwjRqHMYC0pmP22fEnEYojirCbZf3rEfNKX6Nhm_uiVbi1rcz1H7aHq6Tt03chjYlPOKOXrATzjPQbNGhCoyDKrsy1fwCyG5aC2Y1GoQNO6pNiNKILiU4AvSO8o6R1WwMaiqB3Kgs9lmrly1alYZG9tbSlOh4u6M3_31dxMyXCnXRN5vRz7uhTe5GM3mcD4Dbzw5n41IuqSDhPssKZ3T6MXVTpmT4FSFTWHw9k9g14ryEOtRWXGNg1nwQJRhZb6CFBXTNl6T80zFmwfDXLBMapZ5dlZtfSkHKVsNMSEzC-ayCTkdh5Q_X8347cuZcXm-rrdqQpbkZzmKVmlvMol9LJ3_Yp35JD2C07qJ4jBq3TLNS8rE8nlGpuR9BBi0lkg4mlsyMrC1-5WWYQa_8RxL1e63uH6dPMRrOPCyCMlUIK_QruissvaczaENC7ZpLQ30WtsfUO9CpEtR-KoQ2NjJw2OwR2PZowYUytwr1w0IVe2jlMbktIMxGs7b0vWMZJP1S1zfPT-bno6M925ZjGGWZj7jtwkGIRpX2PiBy9NcC6qkecgnLH0o6jszev41ZfoDlJ3ZWaLM48XcI3_e3h7s7h1hPPPU2ap9qyHfiWSnzVIRVtMKRRxYDrUVm1aKImX60rrT8ZeWijDFS8uhrwFeM_1aY11m97SNXv8-cp7WO8lWw8hyrlki6arISTv1S-tmlSoNULhkwMpy-r1e12SxnDtrZTmd3i977XbnsN856B_1Dztduru2nNZBt7131Dk86NNar9_r9A_um9Zng9zZ67U7nW77oN0-6P58dNS9_x_mr6kB)
...
The hash key promotion is a special kind of split where we move all the items of a specific hash key to its own Durable Object because it's consuming a big chunk of storage in a hash partition.
If that hash key continues to grow, even a hash split would not help, since all those items would move along with their hash key.
Therefore, we make a root range partition that can execute range partition splits over its sort keys in the future.

For each hash key stored in a hash partition we track "estimated size bytes" which is updated on every write operation.
After each write operation, we compare this estimated size against the configured threshold, and if it's breaching we schedule a promotion.

The process for the hash key promotion shares a lot with the normal split flow shown above.
One difference is that it only involves a single new data partition being created, and the parent partition has to track which hash keys have been promoted into their own root range partitions so that it can forward requests for that hash key.

There are two state machines at play during a hash key promotion:

1. The parent partition being split tracks the promotion status "per hash key":
   1. Once we need a hash key promotion, the status for that hash key is set to `PROMOTION_QUEUED` and starts a background job.
   2. The background job attempts to initialize the new root range partition Durable Object with the right context and only proceeds when initialized. This is idempotent and can be retried if something fails.
   3. The background job will ensure there are no hash splits in-progress (`SPLIT_STARTED` or `SPLIT_QUEUED`) and that the hash key doesn't have a pending transaction. We retry later if needed.
   4. Once the new partition is initialized, status moves to `PROMOTION_PROMOTING`. From this point forward, all item operation requests for that hash key are forwarded to the new root range partition.
   5. Once the new partition migrated their data and acknowledged their completion, status moves to `PROMOTION_COMPLETED` and can cleanup data no longer needed.
2. The new root range partition created tracks the promotion in `migration_status` (same as any other split):
   1. When the parent initializes the new DO, status is set to `MIGRATION_INITIALIZED` and waits for the signal from the parent to start migrating data.
   2. Once notified to migrate data, it sets `MIGRATION_MIGRATING` and starts a background job that will start pulling data. During this status all requests are handled in the child partition: writes are rejected, whereas reads are re-routed back to the parent again and read from there, since we don't yet have the full data locally.
   3. Once all data is migrated, migration status is set to `MIGRATION_COMPLETE`, it signals the parent as complete, and going forward processes all requests locally.

All the operations in the split or hash key promotion flows are idempotent and can be retried at any point in case any of the Durable Objects involved are restarted.
This does mean there is a period with downtime for write operations while the new partitions are pulling their data from the parent partition.
...
Any range based only splitting also has memory issues in our case, since, even though encoding text into Perkle/Patricia trees would save a lot of storage, with thousands of these keys it would still be several MBs.

### FokosDB encoded topology

**NOTE:** This encoding and caching layer is still work-in-progress and not merged in the code. I do quite like the simplicity of the hash partition encoding, but I might change how I encode the range partition trees if I find a better approach.
...
### Partial caching with response piggybacking

In addition to the globally cached topology encoding described in the previous section, I will also implement partial caching within each partition piggybacking on each request's response.

This is an optimization to not depend entirely on the `TopologyKeeperDO` publishing the encoded topology in Workers KV, which also leads to faster discovery of splits in ancestor partitions.

When a hash partition forwards a request to a child partition, then that partition can also forward the request to its own child partition and so on.
When the final hash partition processes the operation, it will include its depth in the response (`hashDepth`), and all partitions that forwarded this request will inspect the response and extract that information.

Due to the deterministic way of routing hash keys in our partition tree, these ancestor partitions that forwarded the request can populate their local topology cache with all the splits relative from their depth in the tree until `hashDepth`.
This means that subsequent requests can be forwarded deeper in the tree instead of just in their child partitions.

The two approaches complement each other and compose together nicely.
...
4. The post-split latencies are low again but more elevated than pre-split, which makes total sense, since now we are doing two hops for each call. Remember, in this load test I did not have the topology caching enabled yet.
   - That spike around 500ms was an unforunate transient timeout.
...
- 2026-06-13: Updated the [partition topology diagram](#system-architecture-overview) with a range split and also different coloring for hash vs range partitions.
- 2026-06-08: Added [Partial caching with response piggybacking](#partial-caching-with-response-piggybacking) section.
