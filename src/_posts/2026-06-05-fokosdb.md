...
I wrote a basic [k6 script](https://grafana.com/docs/k6/latest/) that calls `putItem` for a 10KB text payload, random hash and sort keys in order to be well spread out, and then calls `getItem` for the same keys asserting that the response is the data we wrote.
