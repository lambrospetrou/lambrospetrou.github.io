...
My first instinct was to do something like BigTable, each tablet would be an item in [Workers KV](https://developers.cloudflare.com/kv/concepts/how-kv-works/) and each request starts reading from the root item which should in theory be heavily cached in the request's colo location, and then continue reading more items from Workers KV as necessary to resolve the final target Durable Object partition.
For a database with thousands of partitions this would require several MBs to be fetched by KV on every request (or every couple if we use in-memory cache too), and it would be quite slow eventually for larger databases.
...
- Data migration is done in chunks of 20-25MB. I could do it in bigger chunks with streaming and compression, but I postponed it hoping for a platform native fork API.
