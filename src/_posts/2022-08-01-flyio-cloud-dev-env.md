...
The next 8 lines create a new user `clouddevuser` ([customisable argument using `ARG`](https://docs.docker.com/engine/reference/builder/#arg)) and sets the necessary permissions, and creates the user's home under `/data/home` (we can change this to `/home/clouddevuser` or anything else). The thing to remember from these lines is that export an environment variable `USER_ARG` which holds the user name since it is used by the `entrypoint.sh` script.

Then, we have 2 lines essentially making sure the our local SSH configuration is copied into the container and the one used.
...
The two `COPY` instructions copy the local `entrypoint.sh` script, and also the local directory `./docker-entrypoint.d/`, to the container's root directory.
The last pair of instructions denote that we want our container to basically execute `./entrypoint.sh /usr/sbin/sshd -D` using the [`ENTRYPOINT`](https://docs.docker.com/engine/reference/builder/#entrypoint) and [`CMD`](https://docs.docker.com/engine/reference/builder/#cmd) Dockerfile instructions.
...
This step needs to run only once, in order to create your Fly.io application.
...
If you want to use a specific application name (since it's part of the DNS name you will need to use), then you can use the `--name` argument instead (e.g. `flyctl launch --name lambros-application-1 --no-deploy --copy-config`), but keep in mind that this should be unique across all Fly.io names since it's part of the DNS subdomain you get, e.g. `lambros-application-1.fly.dev`.
...
The default configuration only allows SSH-ing with authorized keys, and the password SSH-ing is disabled (see `etc/ssh/sshd_config` in the source code).
...
2. The default configuration only allows SSH-ing with authorized keys, and the password SSH-ing is disabled (see `etc/ssh/sshd_config` in the source code).
Therefore, you need to update the `HOME_SSH_AUTHORIZED_KEYS` value in `fly.toml` with your laptop's SSH key (usually `~/.ssh/id_rsa.pub`), and then deploy once with `flyctl deploy`. Then, you can remove the SSH key from the `HOME_SSH_AUTHORIZED_KEYS` again to keep it safe.
