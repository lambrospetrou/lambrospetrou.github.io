...
import { getAllNoteSlugs, getNoteData } from "../../lib/posts-store";
...
export default function Note({ postData }) {
  ...
  if (canonicalUrl === "") {
    canonicalUrl = `https://www.lambrospetrou.com/notes/${slug}/`;
  }
  ...
  return (
    <Layout>
      <Head>
        ...
        <title>{title} | Note by Lambros Petrou</title>
        <meta property="og:title" content={`${title} | Notes by Lambros Petrou`} />
...
export async function getStaticProps({ params, preview }) {
  const postData = getNoteData(params.slug, {
    reloadPosts: preview,
  });
...
export async function getStaticPaths() {
  const paths = getAllNoteSlugs().map((slug) => ({
    params: { slug },
  }));
  return {
    paths,
