...
function Item({item}) {
  const { url, title, author, type, tags } = item;
  const hostname = getHostname(url);
  const isFavourite = tags?.indexOf("favourite") > -1;
  const id = getItemId(item);

  return <li>
    <p>
      <Icon className="mr-10" type={type}/>
      <a className="chain-link" href={`#${id}`}><Icon type="chain-link"/></a>{" "}
      <Aex id={id} href={url}>{title}</Aex>
      {isFavourite && <>{" "}<Icon type="favourite"/></>} - {author}{" "}
